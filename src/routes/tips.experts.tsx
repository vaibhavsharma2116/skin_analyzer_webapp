...
import { ANSWERS, EXPERTS, EXPERTS_BY_ID } from "@/lib/tips-content";
...
function ExpertsPage() {
  ...
  const experts = EXPERTS.filter((e) => {
  ...
  const popular = ANSWERS.slice(0, 3);
  ...
  return (
    ...
    >
          ...
          >
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 flex items-center gap-1 text-[11px] font-semibold">
                ...
                {e.rating} <span className="text-muted-foreground">({e.answers} Answers)</span>
              </p>
            </div>
      ...
      <div className="mt-2 space-y-2">
        {popular.map((a) => {
          const expert = EXPERTS_BY_ID[a.expertId];
          return (
            <button
              key={a.id}
              type="button"
              onClick={() => navigate({ to: "/tips/experts/$id", params: { id: a.expertId } })}
